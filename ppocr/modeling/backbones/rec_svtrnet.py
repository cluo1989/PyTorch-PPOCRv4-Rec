...
import numpy as np
from ppocr.modeling.commons.activations import Activation
...
def drop_path(x, drop_prob=0., training=False):
    ...
    output = x.divide(keep_prob) * random_tensor.to(x.device) # cpu/gpu
    return output
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.mixer == 'Local':
            self.mask = self.mask.to(x.device) # cpu/gpu
            attn += self.mask
    ...
    def __init__(
        ...
        if last_stage:
            ...
            self.last_conv = nn.Conv2d(
                ...
                padding=0,
                bias=False)
            self.hardswish = Activation('hardswish', inplace=True) #nn.Hardswish()
        ...
        if use_lenhead:
            ...
            self.hardswish_len = Activation('hardswish', inplace=True)# nn.Hardswish()
...
if __name__ == '__main__':
    ...
    backbone = SVTRNet(img_size=[32, 320])
    summary(backbone, input_size=(3, 32, 320), batch_size=1)
