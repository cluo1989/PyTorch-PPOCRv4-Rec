...
from ppocr.modeling.backbones.rec_svtrnet import Mlp
...
class Transformer(nn.Module):
    ...
    def forward_test(self, src):
        ...
        for len_dec_seq in range(1, self.max_len):
            ...
            if torch.equal(
                    preds_idx,
                    torch.full(preds_idx.shape, 3, dtype=torch.int64)
            ):
                break
    ...
    def forward_beam(self, images):
        ...
        def collect_active_part(
            ...
            beamed_tensor = beamed_tensor.index_select(0, curr_active_inst_idx) #(curr_active_inst_idx, axis=0)
    ...
    def generate_square_subsequent_mask(self, sz):
        ...
        mask_inf = torch.triu(
            torch.full(size=[sz, sz], fill_value=float('-Inf'),  
            dtype=torch.float32), 
            diagonal=1
        )
        mask = mask + mask_inf
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.norm3 = nn.LayerNorm(d_model, epsilon)
